import { fromHex, C as LCore, Data, Constr, Lucid } from 'lucid-cardano'

import { validate } from '../nft-toolkit/utils.js'

function datumToHash(bytes: Uint8Array) {
...
export class JpgStore {
  ...
  static async #getTxnInfoFromBlockfrost(
    txHash: string,
    resource: string,
    lucid: Lucid
  ) {
  ...
  static async getInputUtxo(metadata, payees, lucid) {
    ...
    const inputTxn = await JpgStore.#getTxnInfoFromBlockfrost(
      txHash,
      'utxos',
      lucid
    )
    const relevantOutput = JpgStore.#findRelevantOutput(
      inputTxn,
      metadata.assetId
    )

    const txnMetadata = await JpgStore.#getTxnInfoFromBlockfrost(
      txHash,
      'metadata',
      lucid
    )
    const datum = fromHex(
      txnMetadata.map((elem) => elem.json_metadata).join('')
    )
    const datumHash = datumToHash(datum)
    validate(
      relevantOutput.data_hash === datumHash,
      `Expected ${datumHash}, found ${relevantOutput.data_hash}`
    )
  ...
  static ConstrDatumFor(payees, lucid) {
    var payeeSchemas = []
    ...
    const marketFeeDetails = lucid.utils.getAddressDetails(
      payees.market_fee.addr
    )
    payeeSchemas.push(
      JpgStore.#paymentSchema(
    ...
    const data = Data.to(
      new Constr(0, [sellerDetails.paymentCredential.hash, payeeSchemas])
    )
    return fromHex(data)
  }
  ...
  static #paymentSchema(
    ...
    return new Constr(0, [
      new Constr(0, [
        new Constr(0, [paymentAddr]),
        new Constr(0, [new Constr(0, [new Constr(0, [stakeKey])])]),
      ]),
      new Map([
        [
          currencyPolicy,
